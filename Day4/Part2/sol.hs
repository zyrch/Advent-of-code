...
solve [] _ = 0
solve (x:xs) matrices = if (length newMatrices == 1 && (checkMatrix $ head newMatrices)) then x * (getUnmarkedSum $ head $ newMatrices) else solve xs remMatrices
  where newMatrices = map (markMatrix x) matrices
        remMatrices = filter (\p -> checkMatrix p == False) newMatrices
